...
app.get("/controleur", (req, res) => {
  ...
  mysql.db.query("SELECT * FROM gods ORDER BY name", (err, result, fields) => {
    if (err) throw err;
    res.render('controleur', {
      title: 'Controller',
      message: 'Ceci est la partie controleur',
      allGods: result
    });
  });
});
...
app.use((err, req, res, next) => { //clientErrorHandler
  if (req.xhr) {
    res.status(500).send({
      error: 'Un truc ne marche pas :' + err
    });
  } else {
    next(err);
  }
